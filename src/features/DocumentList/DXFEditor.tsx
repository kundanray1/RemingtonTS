...
//     const points = [];
//     const angle = 4 * Math.atan(bulge); // Central angle of the arc
//     const distance = Math.sqrt((end.x - start.x) ** 2 + (end.y - start.y) ** 2); // Distance between start and end points
//     const radius = distance / (2 * Math.sin(angle / 2)); // Radius of the arc
...
//     const direction = bulge > 0 ? 1 : -1; // Positive bulge: clockwise, Negative: counter-clockwise
...
// Enhanced Arc Calculation
// Helper function to calculate arc points (handles bulges in polylines)
const calculateArcPoints = (start, end, bulge) => {
    const points = [];
    if (bulge === 0) return points; // Straight line if bulge is 0
...
const renderDXF = (helper) => {
    ...
    entities.forEach((entity) => {
        ...
        try {
            if (entity.type === 'POLYLINE' && Array.isArray(entity.vertices)) {
                const points = [];
